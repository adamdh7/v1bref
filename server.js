...
function contentTypeFromName(filename) {
    ...
    const map = {
        ...
        '.ts': 'video/mp2t', '.mp3': 'audio/mpeg', '.wav': 'audio/wav', '.ogg': 'audio/ogg',
        '.m4a': 'audio/mp4', '.pdf': 'application/pdf', '.json': 'application/json', '.txt': 'text/plain',
...
function isImageFile(filename) {
    return ['.png', '.jpg', '.jpeg', '.gif', '.webp', '.bmp', '.svg'].includes(path.extname(filename).toLowerCase());
}
...
function isDirectVideoFile(filename) {
    return ['.mp4', '.webm', '.m4v'].includes(path.extname(filename).toLowerCase());
}

function needsTranscode(filename) {
    return ['.mov', '.mkv', '.avi', '.wmv', '.flv', '.3gp', '.ts', '.mpeg', '.mpg', '.m2ts'].includes(path.extname(filename).toLowerCase());
}
